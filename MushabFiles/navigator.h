...
public:
    ...
    void head(bool north_rb, bool east_rb, bool south_rb, bool west_rb,int coord_x, int coord_y){
        int i = index(coord_x, coord_y);
        if (map[i]) {return;}
    ...
    void updateMap(bool north_rb, bool east_rb, bool south_rb, bool west_rb){
        double x = odem.getGrid(odem.getX());
        double y = odem.getGrid(odem.getY());
        if ((fmod(x,CELL_SIZE) < L_ERROR) &&(fmod(y,CELL_SIZE)< L_ERROR)){
            int gridX = trunc(x,CELL_SIZE);
            int gridY = trunc(y,CELL_SIZE);
    ...
    Vec2<int> getDest() const {
        double dest_x = X_check % 12;
        double dest_y = Y_check % 12;
