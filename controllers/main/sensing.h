...
private:
  ...
  // Used by is2SquaresLeftGreen / RightGreen to scan specific parts of the image
  ...
  FloorColor processVision() {
    ...
    const unsigned char *image = camera->getImage();
    if (!image) return COLOR_NONE;

    int w = camera->getWidth();
    int h = camera->getHeight();
    if (w < 5 || h < 5) return COLOR_NONE;
    ...
    if (r > g + 40 && r > b + 40) return COLOR_RED;
...
public:
  ...
  void update() {
    ...
    if (gyro) {
      ...
      if (!std::isnan(values[2])) {
        ...
        if (std::abs(velocity) < 0.002) velocity = 0.0; // Deadband
  ...
  double getDistanceToFrontWall() {
    ...
    for (int k = 0; k < 2; k++) {
        int i = ids[k];
        double rawIR = distanceSensors[i]->getValue();
        double ambientLight = (i < (int)lightSensors.size()) ? lightSensors[i]->getValue() : 0.0;
  ...
  bool isWallGreen() {
    return (processVision() == COLOR_GREEN);
  }
